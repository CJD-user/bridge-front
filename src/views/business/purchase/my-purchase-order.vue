  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        <a-button @click="addOrder" type="primary" v-privilege="'purchaseOrder:add'">
          <template #icon>
            <PlusOutlined />
          </template>
          发起采购申请
      ...
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            <a-button v-if="record.approvalStatus === 1" @click="withdrawOrder(record)" type="link" danger v-privilege="'purchaseOrder:update'">撤回</a-button>
    ...
    <PurchaseOrderFormModal ref="formModal" @reloadList="queryData" />
    <PurchaseOrderDetailModal ref="detailModal" />
  </a-card>
</template>
<script setup>
  import PurchaseOrderFormModal from './components/purchase-order-form-modal.vue';
  ...
  async function queryData() {
    try {
      let queryResult = await purchaseOrderApi.queryMy(queryForm);
  ...
  function withdrawOrder(record) {
    Modal.confirm({
      ...
      okType: 'danger',
      onOk() {
        doWithdraw(record.purchaseOrderId);
      },
      cancelText: '取消',
    });
  }

  async function doWithdraw(purchaseOrderId) {
    try {
      SmartLoading.show();
      await purchaseOrderApi.withdraw(purchaseOrderId);
